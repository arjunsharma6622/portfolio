...
const SecondaryNavigation = () => {
  ...
  return (
    <div className={`text-white text-base ${showSecondaryNav ? '' : 'hidden'} duration-300`}>
      <nav className="fixed -left-2 top-1/2 transform -translate-y-1/2 flex flex-col items-center bg-primary border-primary text-white py-1 -px-2">
      <a href="#contact" className={`text-accent1 hover:text-accent2 transform -rotate-90 my-10 ${activeSection === 'contact' ? 'text-blue-600' : ''}`}>
          <span className={`underline-animation ${activeSection === 'contact' ? 'active' : ''}`}>Contact</span>
        </a>
        <a href="#projects" className={`text-accent1 hover:text-accent2 transform -rotate-90 my-10 ${activeSection === 'projects' ? 'text-blue-600' : ''}`}>
          <span className={`underline-animation ${activeSection === 'projects' ? 'active' : ''}`}>Projects</span>
        </a>
        <a href="#experience" className={`text-accent1 hover:text-accent2 transform -rotate-90 my-10 ${activeSection === 'experience' ? 'text-blue-600' : ''}`}>
          <span className={`underline-animation ${activeSection === 'experience' ? 'active' : ''}`}>Experience</span>
        </a>
        <a href="#skills" className={`text-accent1 hover:text-accent2 transform -rotate-90 my-10 ${activeSection === 'skills' ? 'text-blue-600' : ''}`}>
          <span className={`underline-animation ${activeSection === 'skills' ? 'active' : ''}`}>Skills</span>
        </a>
        <a href="#about" className={`text-accent1 hover:text-accent2 transform -rotate-90 my-10 ${activeSection === 'about' ? 'text-blue-600' : ''}`}>
